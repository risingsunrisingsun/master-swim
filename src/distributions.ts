 ...
 * ## 아직 의심스러운 곳
 *
 * **50대 네 분포 전부**(`M/50/free`, `F/50/free`, `M/50/back`, `F/50/back`)에서
 * **상위 5% 커트라인이 20~40대보다 빠르게** 나온다. 중앙값은 연대 순서를
 * 지키므로(테스트로 고정) 실사용에 큰 지장은 없지만, 상위 5% 부근 표시는 이 네
 * 분포에서 신뢰도가 낮다.
 *
 ...
 * 원본을 의심하는 근거 두 개:
 *
 * - **50대 그래프가 40대보다 왼쪽에서 시작한다** — 네 조합 모두. 자유형 남자는
 *   40대가 25초, 50대가 23초에서 시작한다. 50대에 더 빠른 사람이 있다는 뜻이 되어
 *   연대 순서와 반대다.
 * - **40대에는 같은 평지가 없다.** "은퇴한 경영 선수가 50대에 섞였다"는 설명이라면
 *   40대에도 나타나야 한다. 50대에서만, 그리고 네 조합 전부에서 나타나는 것은
 *   원본의 집계 방식(릴레이 구간·특별부 포함 등)을 의심하게 만든다.
 *
 * 확인할 길은 분위수 표를 직접 받는 것뿐이다. 그때까지 이 네 분포의 상위 5% 표시는
 * 믿지 않는다.
 *
